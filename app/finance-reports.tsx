...
function ReportMovement({ item, busy, onOpen }: { item: AnalyticsDetail; busy: boolean; onOpen: () => void }) {
  const reversal = item.entityType === "expected_income_operation" && item.amountCents < 0;
  const income = item.type === "income" && !reversal;
  return <button type="button" disabled={busy} onClick={onOpen} className="block w-full min-w-0 py-4 text-left transition hover:bg-[#f8faf9] disabled:opacity-60"><div className="flex min-w-0 items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${income ? "bg-[#eff8e9] text-[#39742c]" : "bg-[#fff1df] text-[#95591d]"}`}>{income ? <ArrowDownLeft className="h-4 w-4" /> : <ArrowUpRight className="h-4 w-4" />}</span><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3"><div className="min-w-0"><p className="font-medium sm:truncate">{item.description}</p><p className="mt-1 text-xs text-[#748681]">{dateLabel(item.date)}</p></div><div className="flex shrink-0 items-center gap-2"><strong className={income ? "text-[#39742c]" : "text-[#965a1d]"}>{income ? "+" : "−"} {brl(Math.abs(item.amountCents))}</strong><ChevronRight className="h-4 w-4 text-[#96a6a2]" /></div></div><div className="mt-3 flex flex-wrap gap-2 text-xs text-[#627872]"><Badge variant="outline">{reversal ? "Estorno de entrada" : income ? "Entrada" : "Despesa"}</Badge><span className="rounded-full bg-[#f1f5f3] px-2.5 py-1">{item.categoryName}</span><span className="rounded-full bg-[#f1f5f3] px-2.5 py-1">{item.subcategoryName}</span><span className="rounded-full bg-[#f1f5f3] px-2.5 py-1">{item.accountName}</span><span className="rounded-full bg-[#f1f5f3] px-2.5 py-1">{item.responsibleName}</span>{item.entityType === "card_installment" && <span className="rounded-full bg-[#eee8ff] px-2.5 py-1 text-[#6848a3]">Cartão · parcela {item.installmentNumber}/{item.installmentCount}</span>}</div></div></div></button>;
}
...
function ReadonlyMovementDialog({ item, onOpenChange }: { item: AnalyticsDetail | null; onOpenChange: (open: boolean) => void }) {
  ...
  const reversal = item.entityType === "expected_income_operation";
  return <Dialog open onOpenChange={onOpenChange}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>{item.description}</DialogTitle><DialogDescription>{card ? "Parcela de cartão exibida somente para consulta. Edite a compra na área de cartões." : reversal ? "Estorno imutável de entrada prevista exibido somente para consulta." : "Pagamento de conta exibido somente para consulta. Edite a conta na área de vencimentos."}</DialogDescription></DialogHeader><div className="grid gap-3 rounded-2xl bg-[#f5f8f6] p-4 text-sm sm:grid-cols-2"><ReadonlyField label="Valor" value={`${item.type === "income" && !reversal ? "+" : "−"} ${brl(Math.abs(item.amountCents))}`} /><ReadonlyField label="Data" value={dateLabel(item.date)} /><ReadonlyField label="Categoria" value={item.categoryName} /><ReadonlyField label="Subcategoria" value={item.subcategoryName} /><ReadonlyField label="Conta" value={item.accountName} /><ReadonlyField label="Responsável" value={item.responsibleName} />{card && <ReadonlyField label="Parcela" value={`${item.installmentNumber}/${item.installmentCount}`} />}</div></DialogContent></Dialog>;
}
