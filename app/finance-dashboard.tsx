...
function DashboardSkeleton({ selection, onSelection }: { selection: FinancePeriodSelection; onSelection: (value: FinancePeriodSelection) => void }) {
  return <section className="space-y-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><Skeleton className="h-4 w-40" /><Skeleton className="mt-3 h-8 w-72 max-w-full" /></div><div className="w-full xl:max-w-3xl"><FinancePeriodFilter value={selection} onChange={onSelection} disabled /></div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{Array.from({ length: 4 }, (_, index) => <Skeleton key={index} className="h-44 rounded-[22px]" />)}</div><div className="grid gap-5 xl:grid-cols-[1.35fr_.85fr]"><Skeleton className="h-96 rounded-[24px]" /><Skeleton className="h-96 rounded-[24px]" /></div></section>;
}
