...
import { ArrowLeft, CalendarClock, ChevronLeft, ChevronRight, CreditCard, Pencil, Link2, Plus, Receipt, RotateCcw, Sparkles, Trash2, XCircle } from "lucide-react";
...
export function CardsView({ data, accounts, categories, onChanged, financial, onFinancialRefresh }: { data: AdvancedSnapshot; accounts: Account[]; categories: Category[]; onChanged: () => Promise<void>; financial?: FinancialController; onFinancialRefresh?: FinancialRefresh }) {
  ...
  const removeCard = async (item: Card) => { try { const result = await advancedApi({ action: "delete_card", id: item.id }); toast.success(result.inactivated ? "Cartão inativado para preservar o histórico." : "Cartão excluído."); await onChanged(); } catch (error) { toast.error(error instanceof Error ? error.message : "Falha."); } };
  ...
  return <section>
    {!card ? <>
      ...
      <div className="flex min-w-0 flex-col justify-between gap-4 lg:flex-row lg:items-start"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="break-words text-3xl font-semibold tracking-[-.04em]">{card.name}</h2><Badge variant={card.isActive ? "secondary" : "outline"}>{card.isActive ? "Ativo" : "Inativo"}</Badge></div><p className="mt-2 break-words text-[#71837e]">{card.institution} · {card.holder}</p><p className="mt-1 text-sm text-[#71837e]">Fecha dia {card.closingDay} · vence dia {card.dueDay}</p></div><div className="flex flex-wrap gap-2"><CardOnboardingAction key={card.id} card={card} categories={categories} onChanged={onChanged} /><Button variant="outline" onClick={() => { setEditing(card); setCardOpen(true); }}><Pencil className="h-4 w-4" /> Editar</Button><Button variant="outline" onClick={() => void removeCard(card)}><Trash2 className="h-4 w-4" /> {card.usedCents ? "Inativar" : "Excluir"}</Button></div></div>
      ...
      {selectedInvoice ? <section className="mt-6" aria-labelledby="relevant-invoice-title"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h3 id="relevant-invoice-title" className="text-lg font-semibold">{selectedInvoice.id === relevantInvoice?.id ? "Fatura relevante" : "Fatura selecionada"}</h3>{selectedInvoice.id !== relevantInvoice?.id && relevantInvoice && <Button type="button" variant="ghost" onClick={() => setSelectedInvoiceId(relevantInvoice.id)}>Voltar à fatura relevante</Button>}</div><InvoiceLifecycle key={selectedInvoice.id} invoice={selectedInvoice} accounts={accounts} onChanged={onChanged} financial={financial} onFinancialRefresh={onFinancialRefresh} /></section> : <p className="mt-6 rounded-2xl border bg-white py-8 text-center text-sm text-[#71837e]">Nenhuma fatura neste cartão.</p>}
