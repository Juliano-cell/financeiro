...
export function CardsView({ data, accounts, categories, onChanged, financial, onFinancialRefresh }: { data: AdvancedSnapshot; accounts: Account[]; categories: Category[]; onChanged: () => Promise<void>; financial?: FinancialController; onFinancialRefresh?: FinancialRefresh }) {
  ...
  return <section><Heading title="Cartões" text="Limites, compras, parcelas e faturas sem duplicar despesas." action={<div className="flex gap-2"><Button variant="outline" onClick={() => { setClassification({ categoryId: null, subcategoryId: null }); setPurchaseOpen(true); }} disabled={!data.cards.some((item) => item.isActive)}><Receipt className="h-4 w-4" /> Compra</Button><Button onClick={() => { setEditing(null); setCardOpen(true); }}><Plus className="h-4 w-4" /> Cartão</Button></div>} />
    {data.cards.length ? <><div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{data.cards.map((item) => <button key={item.id} onClick={() => setSelected(item.id)} className={`rounded-[24px] border p-5 text-left transition ${selected === item.id ? "border-[#397f72] bg-[#edf6f3]" : "bg-white"}`}><div className="flex justify-between gap-3"><div><p className="font-semibold">{item.name}</p><p className="text-sm text-[#71837e]">{item.institution} · {item.holder}</p></div><Badge variant={item.isActive ? "secondary" : "outline"}>{item.isActive ? "Ativo" : "Inativo"}</Badge></div><div className="mt-5 grid grid-cols-2 gap-3 text-sm"><Stat label="Limite" value={brl(item.limitCents)} /><Stat label="Utilizado" value={brl(item.usedCents)} /><Stat label="Disponível" value={brl(item.availableCents)} /><Stat label="Fatura do mês" value={brl(item.currentInvoiceCents)} /><Stat label="Próxima fatura" value={brl(item.nextInvoiceCents)} /></div></button>)}</div>{card && <div className="mt-5 rounded-[24px] border bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-semibold">Faturas · {card.name}</h3><p className="text-sm text-[#71837e]">Fecha dia {card.closingDay}, vence dia {card.dueDay} · {card.installmentPurchaseCount} compra(s) parcelada(s)</p></div><div className="flex flex-wrap gap-2"><CardOnboardingAction key={card.id} card={card} categories={categories} onChanged={onChanged} /><Button variant="outline" size="sm" onClick={() => { setEditing(card); setCardOpen(true); }}>Editar</Button><Button variant="outline" size="sm" onClick={async () => { try { const result = await advancedApi({ action: "delete_card", id: card.id }); toast.success(result.inactivated ? "Cartão inativado para preservar o histórico." : "Cartão excluído."); await onChanged(); } catch (error) { toast.error(error instanceof Error ? error.message : "Falha."); } }}><Trash2 className="h-4 w-4" /> {card.usedCents ? "Inativar" : "Excluir"}</Button></div></div><div className="mt-4 grid gap-3">{invoices.length ? invoices.map((invoice) => <InvoiceLifecycle key={invoice.id} invoice={invoice} accounts={accounts} onChanged={onChanged} financial={financial} onFinancialRefresh={onFinancialRefresh} />) : <p className="py-8 text-center text-sm text-[#71837e]">Nenhuma fatura neste cartão.</p>}</div></div>}</> : <Empty icon={CreditCard} text="Cadastre o primeiro cartão da família." />}
