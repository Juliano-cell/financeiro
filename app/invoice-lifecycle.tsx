...
export type CanonicalInvoice = { id: string; cardId: string; referenceMonth: string; dueDate: string; invoiceTotalCents: number; paidCents: number; remainingCents: number; paymentStatus: "unpaid" | "partial" | "settled"; cycleStatus: "open" | "closed" | "unknown"; closesOn: string | null; installments: unknown[] };
...
export function InvoiceLifecycle({ invoice, accounts, onChanged, financial, onFinancialRefresh }: { invoice: CanonicalInvoice; accounts: Account[]; onChanged: () => Promise<void>; financial?: FinancialController; onFinancialRefresh?: FinancialRefresh }) {
  ...
  return <article className="min-w-0 rounded-2xl bg-[#f6f8f7] p-4">
    ...
    <div className="mt-4 flex flex-wrap gap-2">{invoice.remainingCents > 0 && <Button className="min-h-11" disabled={awaitingSnapshot || !accounts.some((account) => account.isActive)} onClick={() => setEditing({ kind: "payment" })}>Pagar restante</Button>}<Button variant="outline" className="min-h-11" aria-expanded={historyOpen} onClick={() => { setHistoryOpen((value) => !value); setHistory(null); setHistoryError(""); }}>Histórico de pagamentos</Button></div>
    ...
    {historyOpen && <div className="mt-4 border-t pt-4" aria-live="polite"><h5 className="font-semibold">Histórico</h5>{historyError ? <div role="alert"><p className="mt-2 text-sm">{historyError}</p><Button variant="outline" onClick={() => setRetry((value) => value + 1)}>Tentar novamente</Button></div> : !history ? <p className="mt-2 text-sm">Carregando histórico…</p> : events.length ? <ul className="mt-3 grid gap-3">{events.map((event) => <li key={event.key} className="min-w-0 rounded-xl border bg-white p-3"><p className="font-medium">{event.label} · {money(event.amountCents)}</p><p className="mt-1 break-words text-sm">{dateLabel(event.date)} · {event.accountName}</p>{event.canReverse ? <Button className="mt-2 min-h-11" variant="outline" disabled={awaitingSnapshot} onClick={() => setEditing({ kind: "reversal", event })}>Reverter pagamento</Button> : event.paymentId && <p className="mt-2 text-xs">Pagamento revertido</p>}</li>)}</ul> : <p className="mt-2 text-sm">Nenhum pagamento registrado.</p>}</div>}
...
export function InvoiceOperationDialog({ invoice, accounts, editing, onClose, onChanged, onInvalidate, financial }: { invoice: CanonicalInvoice; accounts: Account[]; editing: { kind: "payment" } | { kind: "reversal"; event: Event }; onClose: () => void; onChanged: () => Promise<void>; onInvalidate: () => void; financial?: FinancialController }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open && !busy) onClose(); }}><DialogContent className="max-h-[85dvh] overflow-y-auto"><DialogHeader><DialogTitle>{reversal ? `Reverter pagamento de ${money(editing.event.amountCents)}?` : "Pagar fatura"}</DialogTitle><DialogDescription>{reversal ? "O valor retornará para a conta utilizada e a fatura voltará a possuir saldo pendente. O pagamento original será preservado." : "Confira o valor restante e escolha explicitamente a conta. O residual será pago integralmente."}</DialogDescription></DialogHeader>
    <div className="rounded-xl bg-[#f4f7f5] p-4"><p>{reversal ? `Conta: ${editing.event.accountName}` : "Valor restante"}</p><strong className="text-xl">{money(reversal ? editing.event.amountCents : Number(attempt?.expectedRemainingCents ?? invoice.remainingCents))}</strong></div>
    {!reversal && <div><Label htmlFor="invoice-account">Conta que pagará a fatura *</Label><select id="invoice-account" value={accountId} disabled={busy || Boolean(attempt)} onChange={(event) => setAccountId(event.target.value)} className="mt-2 min-h-11 w-full min-w-0 rounded-md border bg-white px-3"><option value="" disabled>Selecione uma conta</option>{accounts.filter((account) => account.isActive).map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></div>}
    ...
    <DialogFooter><Button variant="outline" disabled={busy} onClick={onClose}>Cancelar</Button><Button disabled={busy || !date || (!reversal && !accountId)} onClick={() => void confirm()}>{busy ? "Confirmando…" : attempt ? "Tentar novamente a mesma operação" : reversal ? "Reverter pagamento" : "Confirmar pagamento"}</Button></DialogFooter>
