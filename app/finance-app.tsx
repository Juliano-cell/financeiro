    ...
    <main className="min-h-screen bg-[#f4f6f5] text-[#132b27]">
      ...
      <section className="pb-20 lg:pl-[248px] lg:pb-0">
        ...
        <div className="mx-auto max-w-[1460px] p-4 md:p-8">
          ...
          {view === "categories" && <CategoriesView categories={categories} onNew={() => { setEditingCategory(null); setCategoryOpen(true); }} onEdit={(item) => { setEditingCategory(item); setCategoryOpen(true); }} onDelete={async (item) => { await api({ action: "delete_category", id: item.id }); toast.success("Categoria excluída."); await load(); }} />}
...
function EnhancedTransactionDialog({ open, item, accounts, categories, cards, onOpenChange, onSaved }: { open: boolean; item: Transaction | null; accounts: Account[]; categories: Category[]; cards: Array<{ id: string; name: string; isActive: boolean }>; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setBusy(true);
    try {
      if (isCardPurchase) await advancedApi({ action: "create_card_purchase", cardId: form.get("cardId"), description: form.get("description"), totalCents: amountCents, purchaseDate: form.get("transactionDate"), installmentCount: Number(form.get("installmentCount")), categoryId: classification.categoryId, notes: form.get("notes") || null, origin: "web" });
