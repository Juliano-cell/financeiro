...
function MobileNav({ current, onNavigate }: { current: View; onNavigate: (view: View) => void }) { const items = navItems.filter((item) => ["dashboard", "reports", "forecast", "transactions", "bills", "settings"].includes(item.id)); return <nav className="fixed inset-x-0 bottom-0 z-20 grid min-h-[var(--mobile-nav-offset)] grid-cols-6 border-t bg-white px-1 pt-1.5 pb-[calc(0.375rem+env(safe-area-inset-bottom))] lg:hidden">{items.map(({ id, label, icon: Icon }) => <button key={id} onClick={() => onNavigate(id)} className={`flex min-w-0 flex-col items-center gap-1 rounded-xl py-1.5 text-[10px] ${current === id ? "font-semibold text-[#173c35]" : "text-[#758783]"}`}><Icon className={`h-5 w-5 ${current === id ? "text-[#639b32]" : ""}`} /><span className="max-w-full truncate">{label === "Movimentações" ? "Lançar" : label}</span></button>)}</nav>; }
