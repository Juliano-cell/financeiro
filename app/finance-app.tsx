...
import { BarChart3, Bell, CalendarClock, ChevronRight, CircleDollarSign, CreditCard, Eye, EyeOff, Home, Landmark, LayoutDashboard, Menu, Pencil, Plus, ReceiptText, Search, Settings2, Sparkles, Tags, Trash2, Users, Wallet, X } from "lucide-react";
...
type View = "dashboard" | "reports" | "transactions" | "expected-income" | "accounts" | "categories" | "family" | AdvancedView;
...
type Transaction = { id: string; type: "income" | "expense"; amountCents: number; description: string; categoryId: string | null; subcategoryId: string | null; transactionDate: string; transactionTime: string | null; responsibleUserId: string; accountId: string; paymentMethod: string | null; status: "confirmed" | "pending" | "cancelled"; origin: "dashboard" | "telegram"; notes: string | null; accountName: string | null; categoryName: string | null; responsibleName: string | null; isExpectedIncomeManaged?: boolean; isFutureLegacy?: boolean };
...
export function FinanceApp() {
  const { valuesHidden, toggleFinancialValues } = useUiPreferences();
  ...
  const title = { dashboard: "Visão geral", reports: "Relatórios", transactions: "Movimentações", "expected-income": "Entradas previstas", accounts: "Contas e carteiras", categories: "Categorias", family: "Família", cards: "Cartões", installments: "Parcelas", bills: "Contas e vencimentos", simulator: "Simulador", settings: "Configurações" }[view];
...
function EnhancedTransactionDialog({ open, item, accounts, categories, cards, onOpenChange, onSaved }: { open: boolean; item: Transaction | null; accounts: Account[]; categories: Category[]; cards: Array<{ id: string; name: string; isActive: boolean }>; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  const today = genericTransactionToday();
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!isCardPurchase && transactionDate > today) { toast.error(futureGenericTransactionMessage(type)); return; }
...
function TransactionDialog({ open, item, accounts, categories, onOpenChange, onSaved }: { open: boolean; item: Transaction | null; accounts: Account[]; categories: Category[]; onOpenChange: (open: boolean) => void; onSaved: () => Promise<void> }) {
  ...
  const today = genericTransactionToday();
  const legacyFuture = Boolean(item && item.transactionDate > today);
  const selectType = (nextType: "income" | "expense") => {
    const next = changeTransactionType({ type, ...classification }, nextType, categories);
    setType(nextType);
    setClassification({ categoryId: next.categoryId, subcategoryId: next.subcategoryId });
...
function LoadingScreen() { return <main className="grid min-h-screen place-items-center bg-background"><div className="text-center"><div className="mx-auto grid h-12 w-12 animate-pulse place-items-center rounded-2xl bg-[#b9f47a] text-[#17342d]"><CircleDollarSign className="h-6 w-6" /></div><p className="mt-4 text-sm text-muted-foreground">Carregando as finanças da família…</p></div></main>; }
