...
import { bills, cardInstallments, cardInvoices, creditCards, notificationLog, notificationPreferences, telegramLinks } from "@/db/schema";
...
export async function POST(request: Request) {
  ...
  for (const householdId of householdIds) {
    ...
    const links = await db.select().from(telegramLinks).where(and(eq(telegramLinks.householdId, householdId), eq(telegramLinks.isActive, true)));
    const candidates = [
      ...
      ...invoiceRows.filter((item) => item.householdId === householdId && item.status !== "paid").map((item) => ({ entityType: "invoice", entityId: item.id, label: `Fatura ${cards.find((card) => card.id === item.cardId)?.name ?? "cartão"}`, amountCents: installmentRows.filter((part) => part.invoiceId === item.id && part.status !== "cancelled").reduce((sum, part) => sum + part.amountCents, 0), dueDate: item.dueDate })),
    ];
    for (const item of candidates) {
      ...
      for (const link of links) {
        const [logged] = await db.select().from(notificationLog).where(and(eq(notificationLog.entityType, item.entityType), eq(notificationLog.entityId, item.entityId), eq(notificationLog.eventKey, eventKey), eq(notificationLog.channel, "telegram"), eq(notificationLog.recipientKey, link.telegramUserId))).limit(1); if (logged) continue;
