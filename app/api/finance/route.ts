...
import { accounts, auditLogs, bills, cardPurchases, categories, expectedIncomeOperations, householdInviteTokens, householdMembers, households, recurringBillSeries, subcategories, transactions, users } from "@/db/schema";
...
export async function GET() {
  try {
    ...
    const expectedIncomeOperationRows = await db.select({ transactionId: expectedIncomeOperations.transactionId, operationType: expectedIncomeOperations.operationType, financialDate: expectedIncomeOperations.financialDate, amountCents: transactions.amountCents })
      .from(expectedIncomeOperations).leftJoin(transactions, and(eq(transactions.householdId, expectedIncomeOperations.householdId), eq(transactions.id, expectedIncomeOperations.transactionId)))
      .where(and(eq(expectedIncomeOperations.householdId, householdId), isNotNull(expectedIncomeOperations.transactionId)));
    const expectedIncomeTransactionIds = new Set(expectedIncomeOperationRows.map((item) => item.transactionId).filter((value): value is string => Boolean(value)));
    const billPaymentRows = await db.select({ transactionId: bills.paymentTransactionId }).from(bills).where(and(eq(bills.householdId, householdId), isNotNull(bills.paymentTransactionId)));
    const billPaymentTransactionIds = new Set(billPaymentRows.map((item) => item.transactionId).filter((value): value is string => Boolean(value)));
    const reversalRows = expectedIncomeOperationRows.flatMap((operation) => {
      return operation.operationType === "reverse" && operation.financialDate && operation.amountCents !== null ? [{ financialDate: operation.financialDate, amountCents: operation.amountCents }] : [];
    });
    ...
    const incomeCents = confirmedThisMonth.filter((item) => item.type === "income").reduce((sum, item) => sum + item.amountCents, 0)
      - reversalRows.filter((item) => item.financialDate.startsWith(month)).reduce((sum, item) => sum + item.amountCents, 0);
    ...
    for (const reversal of reversalRows.filter((item) => item.financialDate.startsWith(month))) { const point = daily.get(reversal.financialDate) ?? { date: reversal.financialDate, income: 0, expense: 0 }; point.income -= reversal.amountCents; daily.set(reversal.financialDate, point); }
    const today = genericTransactionToday();
    return NextResponse.json({ setupRequired: false, user: { id: auth.userId, name: auth.displayName, email: auth.email }, household: family, membership, members: memberRows, accounts: accountsWithBalance, categories: categoryRows.map((category) => ({ ...category, subcategories: subcategoryRows.filter((sub) => sub.categoryId === category.id).map((sub) => ({ ...sub, isInUse: usedSubcategoryIds.has(sub.id) })) })), transactions: transactionRows.map((item) => ({ ...item, isExpectedIncomeManaged: expectedIncomeTransactionIds.has(item.id), isFutureLegacy: item.transactionDate > today && !expectedIncomeTransactionIds.has(item.id) && !billPaymentTransactionIds.has(item.id) })), summary: { availableCents, incomeCents, expenseCents, pendingBillsCents: 0, projectedCents: availableCents }, cashflow: [...daily.values()].sort((a, b) => a.date.localeCompare(b.date)) });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "update_transaction") {
      const parsed = transactionInput.extend({ id }).parse(body); const [before] = await db.select().from(transactions).where(and(eq(transactions.id, parsed.id), eq(transactions.householdId, householdId))).limit(1); if (!before) return NextResponse.json({ error: "Movimentação não encontrada." }, { status: 404 }); const [billPayment] = await db.select({ id: bills.id }).from(bills).where(and(eq(bills.householdId, householdId), eq(bills.paymentTransactionId, parsed.id))).limit(1); if (billPayment) return NextResponse.json({ error: "O pagamento de uma conta deve ser alterado pela própria conta." }, { status: 409 }); const [expectedIncomeReceipt] = await db.select({ id: expectedIncomeOperations.id }).from(expectedIncomeOperations).where(and(eq(expectedIncomeOperations.householdId, householdId), eq(expectedIncomeOperations.transactionId, parsed.id))).limit(1); if (expectedIncomeReceipt) return NextResponse.json({ error: "O recebimento histórico de uma receita prevista não pode ser alterado." }, { status: 409 }); const dateViolation = futureGenericTransactionViolation({ type: parsed.type, transactionDate: parsed.transactionDate, today: genericTransactionToday(), existingTransactionDate: before.transactionDate }); if (dateViolation) throw new FinanceValidationError(dateViolation.message, { code: dateViolation.code, planningTarget: dateViolation.planningTarget }); const relationError = await validateTransactionRelations(db, householdId, parsed); if (relationError) return NextResponse.json({ error: relationError }, { status: 400 }); const { id: transactionId, ...values } = parsed; await db.update(transactions).set({ ...values, updatedAt: now() }).where(and(eq(transactions.id, transactionId), eq(transactions.householdId, householdId))); await logChange(db, householdId, auth.userId, "update", "transaction", transactionId, before, values); return NextResponse.json({ ok: true });
    }
    if (action === "delete_transaction") {
      const parsed = z.object({ id }).parse(body); const [before] = await db.select().from(transactions).where(and(eq(transactions.id, parsed.id), eq(transactions.householdId, householdId))).limit(1); if (!before) return NextResponse.json({ error: "Movimentação não encontrada." }, { status: 404 }); const [billPayment] = await db.select({ id: bills.id }).from(bills).where(and(eq(bills.householdId, householdId), eq(bills.paymentTransactionId, parsed.id))).limit(1); if (billPayment) return NextResponse.json({ error: "O pagamento de uma conta deve ser gerenciado pela própria conta." }, { status: 409 }); const [expectedIncomeReceipt] = await db.select({ id: expectedIncomeOperations.id }).from(expectedIncomeOperations).where(and(eq(expectedIncomeOperations.householdId, householdId), eq(expectedIncomeOperations.transactionId, parsed.id))).limit(1); if (expectedIncomeReceipt) return NextResponse.json({ error: "O recebimento histórico de uma receita prevista não pode ser excluído." }, { status: 409 }); await db.delete(transactions).where(and(eq(transactions.id, parsed.id), eq(transactions.householdId, householdId))); await logChange(db, householdId, auth.userId, "delete", "transaction", parsed.id, before); return NextResponse.json({ ok: true });
    }
    ...
    if (error instanceof FinanceValidationError) return NextResponse.json({ error: error.message, ...(error.code ? { code: error.code } : {}), ...(error.planningTarget ? { planningTarget: error.planningTarget } : {}) }, { status: 400 });
