...
export function canIdentifyOpeningBalance(openingResidualCents) {
  return Number.isSafeInteger(openingResidualCents) && openingResidualCents > 0;
}

export function initialExistingInstallmentMode(defaultMode, openingResidualCents) {
  if (defaultMode === "included") return canIdentifyOpeningBalance(openingResidualCents) ? "included" : "";
  return defaultMode === "additional" ? "additional" : "";
}

export function parseOnboardingPreviewResponse(value, expectedCardId) {
  if (!value || typeof value !== "object" || Array.isArray(value) || value.cardId !== expectedCardId || value.eligible !== true) return null;
  if (!validIdentifier(value.cardUpdatedAt) || !/^\d{4}-\d{2}-\d{2}$/u.test(value.today)
    || !/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(value.minimumReferenceMonth)
    || !/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(value.maximumReferenceMonth)
    || !/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(value.suggestedReferenceMonth)
    || !Array.isArray(value.cycles)) return null;
  const cycles = value.cycles.map((cycle) => {
    if (!cycle || typeof cycle !== "object" || Array.isArray(cycle)
      || !/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(cycle.referenceMonth)
      || !/^\d{4}-\d{2}-\d{2}$/u.test(cycle.closesOn)
      || !/^\d{4}-\d{2}-\d{2}$/u.test(cycle.dueOn)
      || !(cycle.state === "open" || cycle.state === "closed" || cycle.state === "future")
      || cycle.requiresClosedCycleConfirmation !== (cycle.state === "closed")) return null;
    return { referenceMonth: cycle.referenceMonth, closesOn: cycle.closesOn, dueOn: cycle.dueOn, state: cycle.state, requiresClosedCycleConfirmation: cycle.requiresClosedCycleConfirmation };
  });
  if (cycles.some((cycle) => cycle === null)) return null;
  return { cardId: value.cardId, cardUpdatedAt: value.cardUpdatedAt, today: value.today,
    minimumReferenceMonth: value.minimumReferenceMonth, maximumReferenceMonth: value.maximumReferenceMonth,
    suggestedReferenceMonth: value.suggestedReferenceMonth, cycles };
}

export function validateInstallmentDraft(value, { originalTotalRequired = true } = {}) {
  const errors = {};
  const originalTotalCents = String(value.originalTotal ?? "").trim() ? parseBrlCents(value.originalTotal) : null;
  ...
  if (originalTotalRequired && originalTotalCents === null) errors.originalTotal = "Informe um valor original válido.";
  if (!originalTotalRequired && String(value.originalTotal ?? "").trim() && originalTotalCents === null) errors.originalTotal = "Informe um valor original válido.";
...
export function buildExistingInstallmentPayload(draft) {
  if (!validIdentifier(draft.cardId)) return { valid: false, error: "Cartão inválido." };
  if (!/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(draft.firstReferenceMonth)) return { valid: false, error: "Selecione uma competência válida." };
  if (draft.mode !== "included" && draft.mode !== "additional") return { valid: false, error: "Escolha como o parcelamento entra na primeira fatura." };
  if (!Number.isSafeInteger(draft.expectedOpeningResidualCents) || draft.expectedOpeningResidualCents < 0 || draft.expectedOpeningResidualCents > MAX_MONEY_CENTS) return { valid: false, error: "Atualize o saldo inicial do cartão." };
  const parsed = validateInstallmentDraft({ ...draft, currentInstallmentNumber: draft.firstOriginalInstallmentNumber }, { originalTotalRequired: false });
  if (!parsed.valid) return { valid: false, error: "Revise os dados do parcelamento.", fieldErrors: parsed.errors };
  const remainingInstallmentCount = parsed.originalInstallmentCount - parsed.currentInstallmentNumber + 1;
  const remainingTotalCents = parsed.installmentAmountCents * remainingInstallmentCount;
  if (!Number.isSafeInteger(remainingTotalCents) || remainingTotalCents > MAX_MONEY_CENTS) return { valid: false, error: "Valor remanescente inválido." };
  return {
    valid: true,
    payload: {
      cardId: draft.cardId,
      firstReferenceMonth: draft.firstReferenceMonth,
      mode: draft.mode,
      expectedOpeningResidualCents: draft.expectedOpeningResidualCents,
      description: String(draft.description).trim(),
      installmentAmountCents: parsed.installmentAmountCents,
      originalInstallmentCount: parsed.originalInstallmentCount,
      firstOriginalInstallmentNumber: parsed.currentInstallmentNumber,
      originalTotalCents: parsed.originalTotalCents,
      originalPurchaseDate: draft.originalPurchaseDate || null,
      categoryId: draft.categoryId || null,
      subcategoryId: draft.subcategoryId || null,
      notes: String(draft.notes ?? "").trim() || null,
    },
    preview: { remainingInstallmentCount, remainingTotalCents },
  };
}

export function parseExistingInstallmentSuccessResponse(value, expectedCardId) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  if (value.cardId !== expectedCardId || !validIdentifier(value.batchId) || !validIdentifier(value.invoiceId)) return null;
  if (!/^\d{4}-(?:0[1-9]|1[0-2])$/u.test(value.firstReferenceMonth)) return null;
  if (value.importedPurchaseCount !== 1 || !validCount(value.importedInstallmentCount) || value.importedInstallmentCount < 1 || value.importedInstallmentCount > 120) return null;
  if (value.status !== "completed" || typeof value.replayed !== "boolean") return null;
  if (!(value.mode === "included" || value.mode === "additional")
    || !Number.isSafeInteger(value.allocatedAmountCents) || value.allocatedAmountCents < 0
    || !Number.isSafeInteger(value.openingOriginalCents) || value.openingOriginalCents < 0
    || !Number.isSafeInteger(value.openingAllocatedCents) || value.openingAllocatedCents < 0
    || !Number.isSafeInteger(value.openingResidualCents) || value.openingResidualCents < 0
    || !Number.isSafeInteger(value.invoiceTotalCents) || value.invoiceTotalCents < 0
    || value.openingAllocatedCents + value.openingResidualCents !== value.openingOriginalCents) return null;
  return {
    cardId: value.cardId,
    batchId: value.batchId,
    invoiceId: value.invoiceId,
    firstReferenceMonth: value.firstReferenceMonth,
    importedPurchaseCount: value.importedPurchaseCount,
    importedInstallmentCount: value.importedInstallmentCount,
    mode: value.mode,
    allocatedAmountCents: value.allocatedAmountCents,
    openingOriginalCents: value.openingOriginalCents,
    openingAllocatedCents: value.openingAllocatedCents,
    openingResidualCents: value.openingResidualCents,
    invoiceTotalCents: value.invoiceTotalCents,
    status: value.status,
...
export function buildOnboardingPayload(draft) {
  ...
  if (!validIdentifier(draft.expectedCardUpdatedAt) || !/^\d{4}-\d{2}-\d{2}$/u.test(draft.expectedClosesOn)
    || !/^\d{4}-\d{2}-\d{2}$/u.test(draft.expectedDueOn)) return { valid: false, error: "Atualize a prévia da fatura." };
  if (draft.requiresClosedCycleConfirmation && draft.closedCycleConfirmed !== true) return { valid: false, error: "Confirme que deseja usar uma fatura cujo ciclo já fechou." };
  ...
  return {
    valid: true,
    payload: { cardId: draft.cardId, referenceMonth: draft.referenceMonth, declaredCurrentInvoiceTotalCents,
      expectedCardUpdatedAt: draft.expectedCardUpdatedAt, expectedClosesOn: draft.expectedClosesOn,
      expectedDueOn: draft.expectedDueOn, closedCycleConfirmed: draft.closedCycleConfirmed === true, existingInstallments },
