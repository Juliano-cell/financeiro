...
function extractDescription(raw, entities) {
  let description = raw.toLocaleLowerCase("pt-BR")
    .replace(/\b(?:e\s+)?(?:vou\s+pagar|pago|ficou\s+para(?:\s+pagar)?|vence|vencimento)\b[\s\S]*$/iu, " ")
    .replace(/\b(direto na loja|credi[aá]rio|parcelado direto|parcelado com a loja|na hora)\b/giu, " ")
...
export function parseTelegramMessage(message, context = {}, options = {}) {
  ...
  const futureContext = /\b(vou pagar|pago|ficou para(?: pagar)?|vence|vencimento)\b/u.test(normalized);
  ...
  const futureSignal = type === "expense" && !directSignal && futureContext && futureDate;
