...
function selectMonetaryAmount(raw) {
  const dates = [...raw.matchAll(/\b(?:\d{4}-\d{2}-\d{2}|\d{1,2}\/\d{1,2}(?:\/\d{4})?)\b/gu)];
  const candidates = [...raw.matchAll(new RegExp(AMOUNT.source, "gi"))].flatMap((match) => {
    const start = match.index;
    const end = start + match[0].length;
    const before = raw.slice(0, start);
    const after = raw.slice(end);
    // Identifiers, dates and installment quantities are not isolated prices.
    if (/[\p{L}\p{N}_/.,-]$/u.test(before) || /^[\p{L}\p{N}_/]/u.test(after) || /^[.,-]\d/u.test(after)
      || /\bdia\s*$/iu.test(before)
      || dates.some((date) => start < date.index + date[0].length && end > date.index)) return [];
    const connector = before.match(/\bpor\s+$/iu);
    return [{ value: match[1], start: connector ? start - connector[0].length : start, end,
      contextual: Boolean(connector || /^r\$/iu.test(match[0]) || /reais?$/iu.test(match[0])) }];
  });
  const contextual = candidates.filter((candidate) => candidate.contextual);
  const eligible = contextual.length ? contextual : candidates;
...
function extractDescription(raw, entities, amount) {
  const withoutAmount = amount ? raw.slice(0, amount.start) + " " + raw.slice(amount.end) : raw;
  let description = withoutAmount.toLocaleLowerCase("pt-BR")
  ...
  description = description
    ...
    .replace(/(?<!\d)\.|\.(?!\d)|[^\p{L}\p{N}\s'.-]/gu, " ").replace(/\s+/g, " ").trim();
  if (description) description = description[0].toLocaleUpperCase("pt-BR") + description.slice(1);
  // Keep the spelling of numeric product labels, e.g. C2, iPhone 15 and USB 2.0.
  for (const match of withoutAmount.matchAll(/\b(?:[\p{L}]+\d+[\p{L}\d]*|[\p{L}]+\s+\d+(?:\.\d+)?)\b/gu)) {
    description = description.replace(new RegExp(escapeRegExp(match[0]), "giu"), match[0]);
  }
...
export function parseTelegramMessage(message, context = {}, options = {}) {
  ...
  const amountMatch = selectMonetaryAmount(raw);
  let amountCents = null;
  try { if (amountMatch) amountCents = parseBrazilianMoney(amountMatch.value); } catch { /* caller asks again */ }
  ...
  const description = extractDescription(raw, [accountMatch.value, cardMatch.value], amountMatch);
