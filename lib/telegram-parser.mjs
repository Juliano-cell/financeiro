...
import { parseTelegramInstallmentCount, resolveTelegramDueDate } from "./telegram-payment-flow.mjs";
...
function extractDescription(raw, entities) {
  let description = raw.toLocaleLowerCase("pt-BR")
    .replace(/\b(vou pagar(?: depois)?|(?:e\s+)?pago (?:m[eê]s que vem|dia\s+\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?)|ficou para(?: pagar)?|vence|vencimento|direto na loja|credi[aá]rio|parcelado direto|parcelado com a loja|na hora)\b/giu, " ")
    .replace(STOP_WORDS, " ").replace(AMOUNT, " ").replace(/\b\d{1,3}\s*x\b/giu, " ");
...
export function parseTelegramMessage(message, context = {}, options = {}) {
  ...
  const parsedInstallmentCount = /\ba vista\b/u.test(normalized) ? 1 : parseTelegramInstallmentCount(normalized);
  const directSignal = /\b(direto na loja|crediario|parcelado direto|parcelado com a loja)\b/u.test(normalized);
  const futureSignal = /\b(vou pagar depois|pago mes que vem|pago dia\s+\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?|ficou para (?:pagar )?(?:mes que vem|dia\s+\d{1,2})|vence dia\s+\d{1,2}|vou pagar dia\s+\d{1,2}(?:\/\d{1,2}(?:\/\d{4})?)?)\b/u.test(normalized);
  const explicitCardSignal = /\b(no cartao|no credito|passei no cartao|cartao de credito)\b/u.test(normalized);
  ...
  const due = paymentFlow === "future_bill" || paymentFlow === "direct_installments" ? resolveTelegramDueDate(raw, purchaseDate) : { status: "absent", date: null, desiredDay: null };
