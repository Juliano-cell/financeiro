...
WITH transaction_events AS (
  ...
  SELECT
    o.household_id,
    o.id,
    'expected_income_operation' AS entity_type,
    o.financial_date AS event_date,
    substr(o.financial_date, 1, 7) AS competence_month,
    'income' AS type,
    -t.amount_cents AS amount_cents,
    'Estorno de entrada prevista · ' || t.description AS description,
    t.category_id,
    t.subcategory_id,
    t.account_id,
    NULL AS card_id,
    NULL AS card_name,
    o.performed_by_user_id AS responsible_user_id,
    'conta_a_receber' AS payment_method,
    'system' AS origin,
    NULL AS installment_number,
    NULL AS installment_count
  FROM expected_income_operations o
  INNER JOIN transactions t
    ON t.household_id = o.household_id AND t.id = o.transaction_id
  WHERE o.operation_type = 'reverse'
), financial_events AS (
  SELECT * FROM transaction_events WHERE household_id = ?
  ...
  SELECT
    ...
    cp.origin,
    CASE
      WHEN cp.origin IN ('web', 'telegram') AND m.purchase_id IS NULL THEN ci.installment_number
      WHEN cp.origin = 'system' AND ib.id IS NOT NULL
        AND typeof(m.first_original_installment_number) = 'integer'
        AND typeof(m.original_installment_count) = 'integer'
        AND m.first_original_installment_number >= 1
        AND m.first_original_installment_number <= m.original_installment_count
        AND ci.installment_number >= 1
        AND ci.installment_number <= ci.installment_count
        AND ci.installment_count = m.original_installment_count - m.first_original_installment_number + 1
        AND m.first_original_installment_number + ci.installment_number - 1 <= m.original_installment_count
      THEN m.first_original_installment_number + ci.installment_number - 1
      ELSE NULL
    END AS installment_number,
    CASE
      WHEN cp.origin IN ('web', 'telegram') AND m.purchase_id IS NULL THEN ci.installment_count
      WHEN cp.origin = 'system' AND ib.id IS NOT NULL
        AND typeof(m.first_original_installment_number) = 'integer'
        AND typeof(m.original_installment_count) = 'integer'
        AND m.first_original_installment_number >= 1
        ...
        AND ci.installment_count = m.original_installment_count - m.first_original_installment_number + 1
        AND m.first_original_installment_number + ci.installment_number - 1 <= m.original_installment_count
      THEN m.original_installment_count
      ELSE NULL
  ...
  LEFT JOIN card_purchase_import_metadata m
    ON m.household_id = cp.household_id AND m.purchase_id = cp.id
  LEFT JOIN card_import_batches ib
    ON ib.household_id = m.household_id AND ib.id = m.import_batch_id AND ib.card_id = cp.card_id AND ib.status = 'completed'
...
function accountBankEventsCte(throughDate = false) {
  const transactionDateFilter = throughDate ? " AND t.transaction_date <= ?" : "";
  const bankDateFilter = throughDate ? " AND bank.event_date <= ?" : "";
  return `WITH account_bank_events AS (
  SELECT
    ...
    t.account_id,
    t.transaction_date AS event_date,
    CASE WHEN t.type = 'income' THEN t.amount_cents ELSE -t.amount_cents END AS signed_amount_cents,
    CASE
      WHEN t.type = 'income' AND t.payment_method = 'conta_a_receber' AND EXISTS (
        SELECT 1 FROM expected_income_operations receipt
        WHERE receipt.household_id = t.household_id AND receipt.transaction_id = t.id AND receipt.operation_type = 'receive'
      ) THEN 'expected_income_receipt'
      ELSE t.type
    END AS event_type,
    t.amount_cents,
    t.category_id,
    ...
    t.responsible_user_id
  FROM transactions t
  WHERE t.household_id = ? AND t.status = 'confirmed'${transactionDateFilter}
  ...
  FROM (
    SELECT
      ...
      p.household_id,
      p.account_id,
      substr(p.paid_at, 1, 10) AS event_date,
      -p.amount_cents AS signed_amount_cents,
      'invoice_payment' AS event_type,
      p.amount_cents,
    ...
    SELECT
      'invoice_payment_operation' AS source_type,
      o.id AS source_id,
      o.household_id,
      o.account_id,
      o.occurred_on AS event_date,
      o.amount_cents AS signed_amount_cents,
      'invoice_payment_reversal' AS event_type,
      o.amount_cents,
      o.created_by_user_id
    FROM invoice_payment_operations o
    WHERE o.kind = 'reversal'

    UNION ALL

    SELECT
      'expected_income_operation' AS source_type,
      o.id AS source_id,
      o.household_id,
      t.account_id,
      o.financial_date AS event_date,
      -t.amount_cents AS signed_amount_cents,
      'expected_income_reversal' AS event_type,
      t.amount_cents,
      o.performed_by_user_id AS created_by_user_id
    FROM expected_income_operations o
    INNER JOIN transactions t
      ON t.household_id = o.household_id AND t.id = o.transaction_id
    WHERE o.operation_type = 'reverse'
  ) bank
  WHERE bank.household_id = ?${bankDateFilter}
)`;
}

// Canonical account events. Consumers add their own period/order clauses.
// Bindings: transaction household, then invoice-event household.
export const ACCOUNT_BANK_EVENTS_CTE = accountBankEventsCte();

export const CURRENT_ACCOUNT_BALANCES_SQL = `
${accountBankEventsCte(true)}, event_totals AS (
  SELECT account_id, SUM(signed_amount_cents) AS net_cents
...
SELECT
  ...
  a.initial_balance_cents
    + COALESCE(e.net_cents, 0) AS current_balance_cents
FROM accounts a
LEFT JOIN event_totals e ON e.account_id = a.id
WHERE a.household_id = ?
ORDER BY a.id
`;

export const ACCOUNT_MOVEMENTS_CTE = `
${ACCOUNT_BANK_EVENTS_CTE}, account_movements AS (
  SELECT
    household_id,
    account_id,
    event_date,
    CASE WHEN event_type = 'expected_income_reversal' THEN signed_amount_cents ELSE amount_cents END AS amount_cents,
    CASE
      WHEN event_type = 'invoice_payment' THEN 'settlement'
      WHEN event_type = 'invoice_payment_reversal' THEN 'settlement_reversal'
      WHEN event_type IN ('income', 'expected_income_receipt', 'expected_income_reversal') THEN 'income'
      ELSE event_type
    END AS type,
    category_id,
    subcategory_id,
