...
WITH transaction_totals AS (
  ...
  WHERE household_id = ? AND substr(paid_at, 1, 10) <= ?
  GROUP BY account_id
)
SELECT
  a.id AS account_id,
  ...
  a.initial_balance_cents
    ...
    - COALESCE(p.paid_cents, 0) AS current_balance_cents
FROM accounts a
...
WITH account_movements AS (
  ...
  SELECT
    ip.household_id,
    ip.account_id,
    substr(ip.paid_at, 1, 10) AS event_date,
    ip.amount_cents,
    'settlement' AS type,
    NULL AS category_id,
    NULL AS subcategory_id,
    ip.created_by_user_id AS responsible_user_id
  FROM invoice_payments ip
  WHERE ip.household_id = ?
)
`;
