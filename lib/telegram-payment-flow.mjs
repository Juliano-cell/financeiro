import { addMonths, daysInMonth, splitInstallments } from "./finance-rules.mjs";
...
function dateForMonthAndDay(month, desiredDay) {
  const actualDay = Math.min(desiredDay, daysInMonth(month));
  return `${month}-${String(actualDay).padStart(2, "0")}`;
}

function nextOccurrenceForDay(purchaseDate, desiredDay) {
  const currentMonth = purchaseDate.slice(0, 7);
  const currentCandidate = dateForMonthAndDay(currentMonth, desiredDay);
  if (currentCandidate > purchaseDate) return currentCandidate;
  return dateForMonthAndDay(addMonths(currentMonth, 1), desiredDay);
}
...
export function resolveTelegramDueDate(text, purchaseDate) {
  if (!isValidTelegramDate(purchaseDate)) return { status: "invalid", date: null, desiredDay: null };
  const raw = String(text ?? "").trim();
  const normalized = normalize(raw);
  const futureSignal = /\b(vou pagar|pago|pagar depois|ficou para|vence|vencimento|mes que vem)\b/u.test(normalized);
  ...
  if (dayMatch) {
    ...
    const month = /\bmes que vem\b/u.test(normalized) ? addMonths(purchaseDate.slice(0, 7), 1) : null;
    const date = month ? dateForMonthAndDay(month, desiredDay) : nextOccurrenceForDay(purchaseDate, desiredDay);
    return { status: "resolved", date, desiredDay };
  }

  if (/\bmes que vem\b/u.test(normalized)) return { status: "incomplete", date: null, desiredDay: null };
...
export function parseTelegramInstallmentCount(value) {
  ...
  if (/[-−]\s*\d{1,3}\s*(?:x\b|parcelas?\b)/u.test(normalized)) return null;
  const match = normalized.match(/(?:\bem\s+)?\b(\d{1,3})\s*x\b/u) ?? normalized.match(/\b(\d{1,3})\s+parcelas?\b/u) ?? normalized.match(/^(\d{1,3})$/u);
...
export function isLegacyTelegramCardPersistence(intent, storedIntent = intent) {
  return effectiveTelegramPaymentFlow(intent) === "credit_card"
    && Boolean(intent?.cardId)
    && (storedIntent?.paymentFlow === undefined || intent?.legacyCardCompatible === true);
}

export function telegramFinancialPersistenceTarget(intent, storedIntent = intent) {
  if (isLegacyTelegramCardPersistence(intent, storedIntent)) return "card_purchase";
  return effectiveTelegramPaymentFlow(intent) === "immediate" ? "transaction" : null;
}
