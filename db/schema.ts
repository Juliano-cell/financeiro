...
export const billInstallmentSeries = sqliteTable("bill_installment_series", {
  id: text("id").primaryKey(), householdId: text("household_id").notNull().references(() => households.id, { onDelete: "cascade" }), description: text("description").notNull(), totalAmountCents: integer("total_amount_cents").notNull(), installmentCount: integer("installment_count").notNull(), firstDueDate: text("first_due_date").notNull(), configuredDay: integer("configured_day").notNull(), categoryId: text("category_id").notNull(), subcategoryId: text("subcategory_id").references(() => subcategories.id), accountId: text("account_id"), notes: text("notes"), idempotencyKey: text("idempotency_key").notNull(), requestFingerprint: text("request_fingerprint").notNull(), createdByUserId: text("created_by_user_id").notNull(), origin: text("origin", { enum: ["web", "telegram", "system"] }).notNull().default("web"), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull(),
}, (table) => [
  unique("bill_installment_series_household_id_unique").on(table.householdId, table.id),
  uniqueIndex("bill_installment_series_household_key_unique").on(table.householdId, table.idempotencyKey),
  index("idx_bill_installment_series_household_created").on(table.householdId, table.createdAt),
  check("bill_installment_series_description_check", sql`length(trim(${table.description})) between 1 and 120`),
  check("bill_installment_series_total_check", sql`${table.totalAmountCents} > 0 and ${table.totalAmountCents} >= ${table.installmentCount}`),
  check("bill_installment_series_count_check", sql`${table.installmentCount} between 2 and 120`),
  check("bill_installment_series_date_check", sql`length(${table.firstDueDate}) = 10 and ${table.firstDueDate} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and ${table.firstDueDate} >= '0001-01-01' and date(${table.firstDueDate}, '+0 days') is not null and date(${table.firstDueDate}, '+0 days') = ${table.firstDueDate}`),
  check("bill_installment_series_day_check", sql`${table.configuredDay} between 1 and 31 and ${table.configuredDay} = cast(substr(${table.firstDueDate}, 9, 2) as integer)`),
  check("bill_installment_series_notes_check", sql`${table.notes} is null or length(${table.notes}) <= 500`),
  check("bill_installment_series_idempotency_check", sql`length(trim(${table.idempotencyKey})) between 1 and 200`),
  check("bill_installment_series_fingerprint_check", sql`length(${table.requestFingerprint}) = 64 and ${table.requestFingerprint} not glob '*[^0-9a-f]*'`),
  check("bill_installment_series_origin_check", sql`${table.origin} in ('web','telegram','system')`),
  foreignKey({ columns: [table.householdId, table.categoryId], foreignColumns: [categories.householdId, categories.id], name: "bill_installment_series_household_category_fk" }),
  foreignKey({ columns: [table.householdId, table.accountId], foreignColumns: [accounts.householdId, accounts.id], name: "bill_installment_series_household_account_fk" }),
  foreignKey({ columns: [table.householdId, table.createdByUserId], foreignColumns: [householdMembers.householdId, householdMembers.userId], name: "bill_installment_series_household_creator_fk" }),
]);
...
}, (table) => [unique("bills_household_id_unique").on(table.householdId, table.id), uniqueIndex("bills_series_due_unique").on(table.recurrenceSeriesId, table.dueDate), uniqueIndex("bills_payment_transaction_unique").on(table.paymentTransactionId), index("idx_bills_household_due_status").on(table.householdId, table.dueDate, table.status), index("idx_bills_household_subcategory_status").on(table.householdId, table.subcategoryId, table.status), index("idx_bills_recurrence_series").on(table.recurrenceSeriesId), check("bills_amount_check", sql`${table.amountCents} > 0`), check("bills_recurrence_check", sql`${table.recurrence} in ('none','monthly')`), check("bills_status_check", sql`${table.status} in ('pending','paid','cancelled')`), check("bills_origin_check", sql`${table.origin} in ('web','telegram','system')`), check("bills_recurrence_link_check", sql`(${table.recurrence} = 'none' and ${table.recurrenceSeriesId} is null and ${table.recurrenceEndDate} is null) or (${table.recurrence} = 'monthly' and ${table.recurrenceSeriesId} is not null)`), foreignKey({ columns: [table.householdId, table.categoryId], foreignColumns: [categories.householdId, categories.id], name: "bills_household_category_fk" }), foreignKey({ columns: [table.householdId, table.accountId], foreignColumns: [accounts.householdId, accounts.id], name: "bills_household_account_fk" }), foreignKey({ columns: [table.householdId, table.recurrenceSeriesId], foreignColumns: [recurringBillSeries.householdId, recurringBillSeries.id], name: "bills_household_series_fk" }), foreignKey({ columns: [table.householdId, table.paymentTransactionId], foreignColumns: [transactions.householdId, transactions.id], name: "bills_household_transaction_fk" }), foreignKey({ columns: [table.householdId, table.createdByUserId], foreignColumns: [householdMembers.householdId, householdMembers.userId], name: "bills_household_creator_fk" })]);

export const billInstallmentOccurrences = sqliteTable("bill_installment_occurrences", {
  householdId: text("household_id").notNull().references(() => households.id, { onDelete: "cascade" }), seriesId: text("series_id").notNull(), billId: text("bill_id").notNull(), installmentNumber: integer("installment_number").notNull(), createdAt: text("created_at").notNull(),
}, (table) => [
  primaryKey({ columns: [table.householdId, table.seriesId, table.installmentNumber], name: "bill_installment_occurrences_pk" }),
  unique("bill_installment_occurrences_bill_unique").on(table.billId),
  index("idx_bill_installment_occurrences_household_bill").on(table.householdId, table.billId),
  check("bill_installment_occurrences_number_check", sql`${table.installmentNumber} >= 1`),
  foreignKey({ columns: [table.householdId, table.seriesId], foreignColumns: [billInstallmentSeries.householdId, billInstallmentSeries.id], name: "bill_installment_occurrences_household_series_fk" }).onDelete("cascade"),
  foreignKey({ columns: [table.householdId, table.billId], foreignColumns: [bills.householdId, bills.id], name: "bill_installment_occurrences_household_bill_fk" }).onDelete("cascade"),
]);

export const expectedIncomeSeries = sqliteTable("expected_income_series", {
  id: text("id").primaryKey(),
  householdId: text("household_id").notNull().references(() => households.id, { onDelete: "cascade" }),
  description: text("description").notNull(),
  expectedAmountCents: integer("expected_amount_cents").notNull(),
  recurrence: text("recurrence", { enum: ["monthly"] }).notNull().default("monthly"),
  configuredDay: integer("configured_day").notNull(),
  startsOn: text("starts_on").notNull(),
  endsOn: text("ends_on"),
  plannedAccountId: text("planned_account_id"),
  categoryId: text("category_id"),
  subcategoryId: text("subcategory_id").references(() => subcategories.id),
  notes: text("notes"),
  isActive: integer("is_active", { mode: "boolean" }).notNull().default(true),
  materializedThroughMonth: text("materialized_through_month").notNull(),
  lastOperationId: text("last_operation_id").notNull(),
  createdByUserId: text("created_by_user_id").notNull(),
  origin: text("origin", { enum: ["web", "telegram", "system"] }).notNull().default("web"),
  createdAt: text("created_at").notNull(),
  updatedAt: text("updated_at").notNull(),
}, (table) => [
  unique("expected_income_series_household_id_unique").on(table.householdId, table.id),
  index("idx_expected_income_series_household_active").on(table.householdId, table.isActive, table.materializedThroughMonth),
  check("expected_income_series_description_check", sql`length(trim(${table.description})) between 1 and 120`),
  check("expected_income_series_amount_check", sql`${table.expectedAmountCents} between 1 and 100000000000`),
  check("expected_income_series_recurrence_check", sql`${table.recurrence} = 'monthly'`),
  check("expected_income_series_day_check", sql`${table.configuredDay} between 1 and 31`),
  check("expected_income_series_starts_on_check", sql`length(${table.startsOn}) = 10 and ${table.startsOn} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and ${table.startsOn} >= '0001-01-01' and date(${table.startsOn}, '+0 days') is not null and date(${table.startsOn}, '+0 days') = ${table.startsOn}`),
  check("expected_income_series_effective_day_check", sql`cast(substr(${table.startsOn}, 9, 2) as integer) = case when ${table.configuredDay} <= cast(strftime('%d', date(substr(${table.startsOn}, 1, 7) || '-01', '+1 month', '-1 day')) as integer) then ${table.configuredDay} else cast(strftime('%d', date(substr(${table.startsOn}, 1, 7) || '-01', '+1 month', '-1 day')) as integer) end`),
  check("expected_income_series_ends_on_check", sql`${table.endsOn} is null or (length(${table.endsOn}) = 10 and ${table.endsOn} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and ${table.endsOn} >= ${table.startsOn} and date(${table.endsOn}, '+0 days') is not null and date(${table.endsOn}, '+0 days') = ${table.endsOn})`),
  check("expected_income_series_month_check", sql`length(${table.materializedThroughMonth}) = 7 and ${table.materializedThroughMonth} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]' and date(${table.materializedThroughMonth} || '-01', '+0 days') = ${table.materializedThroughMonth} || '-01' and ${table.materializedThroughMonth} >= substr(${table.startsOn}, 1, 7) and (${table.endsOn} is null or ${table.materializedThroughMonth} <= substr(${table.endsOn}, 1, 7))`),
  check("expected_income_series_notes_check", sql`${table.notes} is null or length(${table.notes}) <= 500`),
  check("expected_income_series_active_check", sql`${table.isActive} in (0, 1)`),
  check("expected_income_series_origin_check", sql`${table.origin} in ('web','telegram','system')`),
  foreignKey({ columns: [table.householdId, table.plannedAccountId], foreignColumns: [accounts.householdId, accounts.id], name: "expected_income_series_household_account_fk" }),
  foreignKey({ columns: [table.householdId, table.categoryId], foreignColumns: [categories.householdId, categories.id], name: "expected_income_series_household_category_fk" }),
  foreignKey({ columns: [table.householdId, table.createdByUserId], foreignColumns: [householdMembers.householdId, householdMembers.userId], name: "expected_income_series_household_creator_fk" }),
]);

export const expectedIncomeOccurrences = sqliteTable("expected_income_occurrences", {
  id: text("id").primaryKey(),
  householdId: text("household_id").notNull().references(() => households.id, { onDelete: "cascade" }),
  seriesId: text("series_id"),
  occurrenceMonth: text("occurrence_month"),
  description: text("description").notNull(),
  expectedAmountCents: integer("expected_amount_cents").notNull(),
  ...
  unique("expected_income_occurrences_household_id_unique").on(table.householdId, table.id),
  uniqueIndex("expected_income_occurrences_series_month_unique").on(table.householdId, table.seriesId, table.occurrenceMonth).where(sql`${table.seriesId} is not null`),
  uniqueIndex("expected_income_occurrences_transaction_unique").on(table.householdId, table.receivedTransactionId).where(sql`${table.receivedTransactionId} is not null`),
  index("idx_expected_income_occurrences_household_status_date").on(table.householdId, table.status, table.expectedDate),
  index("idx_expected_income_occurrences_household_series").on(table.householdId, table.seriesId),
  index("idx_expected_income_occurrences_household_account").on(table.householdId, table.plannedAccountId),
  check("expected_income_occurrences_description_check", sql`length(trim(${table.description})) between 1 and 120`),
  check("expected_income_occurrences_amount_check", sql`${table.expectedAmountCents} between 1 and 100000000000`),
  check("expected_income_occurrences_date_check", sql`length(${table.expectedDate}) = 10 and ${table.expectedDate} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and ${table.expectedDate} >= '0001-01-01' and date(${table.expectedDate}, '+0 days') is not null and date(${table.expectedDate}, '+0 days') = ${table.expectedDate}`),
  check("expected_income_occurrences_series_check", sql`(${table.seriesId} is null and ${table.occurrenceMonth} is null) or (${table.seriesId} is not null and length(${table.occurrenceMonth}) = 7 and ${table.occurrenceMonth} = substr(${table.expectedDate}, 1, 7))`),
  check("expected_income_occurrences_notes_check", sql`${table.notes} is null or length(${table.notes}) <= 500`),
  check("expected_income_occurrences_status_check", sql`${table.status} in ('pending','received','cancelled')`),
  check("expected_income_occurrences_lifecycle_check", sql`(${table.status} = 'pending' and ${table.receivedTransactionId} is null and ${table.receivedAt} is null and ${table.cancelledAt} is null) or (${table.status} = 'received' and ${table.receivedTransactionId} is not null and ${table.receivedAt} is not null and ${table.cancelledAt} is null) or (${table.status} = 'cancelled' and ${table.receivedTransactionId} is null and ${table.receivedAt} is null and ${table.cancelledAt} is not null)`),
  check("expected_income_occurrences_origin_check", sql`${table.origin} in ('web','telegram','system')`),
  foreignKey({ columns: [table.householdId, table.seriesId], foreignColumns: [expectedIncomeSeries.householdId, expectedIncomeSeries.id], name: "expected_income_occurrences_household_series_fk" }).onDelete("cascade"),
  foreignKey({ columns: [table.householdId, table.plannedAccountId], foreignColumns: [accounts.householdId, accounts.id], name: "expected_income_occurrences_household_account_fk" }),
  foreignKey({ columns: [table.householdId, table.categoryId], foreignColumns: [categories.householdId, categories.id], name: "expected_income_occurrences_household_category_fk" }),
  foreignKey({ columns: [table.householdId, table.receivedTransactionId], foreignColumns: [transactions.householdId, transactions.id], name: "expected_income_occurrences_household_transaction_fk" }),
  foreignKey({ columns: [table.householdId, table.createdByUserId], foreignColumns: [householdMembers.householdId, householdMembers.userId], name: "expected_income_occurrences_household_creator_fk" }),
]);

export const expectedIncomeOperations = sqliteTable("expected_income_operations", {
  id: text("id").primaryKey(),
  householdId: text("household_id").notNull().references(() => households.id, { onDelete: "cascade" }),
  idempotencyKey: text("idempotency_key").notNull(),
  requestHash: text("request_hash").notNull(),
  operationType: text("operation_type", { enum: ["create_occurrence", "create_series", "materialize", "receive", "cancel", "reverse", "update_occurrence"] }).notNull(),
  seriesId: text("series_id"),
  occurrenceId: text("occurrence_id"),
  transactionId: text("transaction_id"),
  performedByUserId: text("performed_by_user_id").notNull(),
  financialDate: text("financial_date"),
  createdAt: text("created_at").notNull(),
}, (table) => [
  unique("expected_income_operations_household_id_unique").on(table.householdId, table.id),
  uniqueIndex("expected_income_operations_household_key_unique").on(table.householdId, table.idempotencyKey),
  index("idx_expected_income_operations_household_occurrence").on(table.householdId, table.occurrenceId, table.createdAt),
  index("idx_expected_income_operations_household_series").on(table.householdId, table.seriesId, table.createdAt),
  index("idx_expected_income_operations_household_transaction").on(table.householdId, table.transactionId, table.operationType),
  uniqueIndex("expected_income_operations_reverse_transaction_unique").on(table.householdId, table.transactionId).where(sql`${table.operationType} = 'reverse'`),
  check("expected_income_operations_key_check", sql`length(trim(${table.idempotencyKey})) between 1 and 200`),
  check("expected_income_operations_hash_check", sql`length(${table.requestHash}) = 64 and ${table.requestHash} not glob '*[^0-9a-f]*'`),
  check("expected_income_operations_type_check", sql`${table.operationType} in ('create_occurrence','create_series','materialize','receive','cancel','reverse','update_occurrence')`),
  check("expected_income_operations_shape_check", sql`(${table.operationType} in ('create_series','materialize') and ${table.seriesId} is not null and ${table.occurrenceId} is null and ${table.transactionId} is null) or (${table.operationType} in ('create_occurrence','update_occurrence','cancel') and ${table.seriesId} is null and ${table.occurrenceId} is not null and ${table.transactionId} is null) or (${table.operationType} in ('receive','reverse') and ${table.seriesId} is null and ${table.occurrenceId} is not null and ${table.transactionId} is not null and ${table.financialDate} is not null)`),
  check("expected_income_operations_date_check", sql`${table.financialDate} is null or (length(${table.financialDate}) = 10 and ${table.financialDate} glob '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]' and ${table.financialDate} >= '0001-01-01' and date(${table.financialDate}, '+0 days') is not null and date(${table.financialDate}, '+0 days') = ${table.financialDate})`),
  foreignKey({ columns: [table.householdId, table.performedByUserId], foreignColumns: [householdMembers.householdId, householdMembers.userId], name: "expected_income_operations_household_user_fk" }),
  foreignKey({ columns: [table.householdId, table.transactionId], foreignColumns: [transactions.householdId, transactions.id], name: "expected_income_operations_household_transaction_fk" }).onDelete("cascade"),
]);
