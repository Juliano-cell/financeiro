...
test("foreign keys compostas rejeitam IDs de outro household", () => {
  ...
  assert.throws(() => db.prepare("INSERT INTO card_installments(id,household_id,purchase_id,invoice_id,installment_number,installment_count,amount_cents,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?)").run("cross-invoice", "ha", "pa", "ib", 1, 1, 100, "pending", at, at), /FOREIGN KEY|same card/iu);
